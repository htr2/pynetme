#!/usr/bin/env python
""""
Using a named regular expression (?P<name>) extract the model from the below show_version string.
...
"""
match = re.search(r"^Cisco (\S+).* with (\S+)", f_str, flags=re.M)
if match:
    model = match.group(1)
    memory = match.group(2)
"""

match = re.search(r"^Cisco (?P<model>\S+).* with (?P<memory>\S+) bytes of memory",f_str, flags=re.M)
...
print()
print("{:20} {:20}".format("Model", model))
print("{:20} {:20}".format("Memory", memory))