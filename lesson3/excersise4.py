...
with open("lesson3/show_arp.txt") as f:
    f_str = f.read()
...
for line in f_str.splitlines():
    ...
    #format mac data into string
    tmp_mac = line_fields[3].split(".")
    tmp_mac = "".join(tmp_mac)
    tmp_mac = tmp_mac.upper()
    #create new mac format
    new_mac = []
    while len(tmp_mac) > 0:
        entry = tmp_mac[:2]
        tmp_mac = tmp_mac[2:]
